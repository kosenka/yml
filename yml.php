...
class Yml
{
    ...
    {
        $allowed = array('typePrefix', 'url', 'price', 'currencyId', 'categoryId', 'picture', 'pickup', 'store', 'delivery', 'local_delivery_cost', 'delivery-options', 'name', 'vendor', 'vendorCode', 'description', 'sales_notes', 'country_of_origin', 'downloadable', 'adult', 'cpa');

        foreach($data as $k=>$v)
        {
            if (!in_array($k, $allowed)) unset($data[$k]);

            if($k=='delivery-options')
                $data[$k] = $this->convert_array_to_attr($v, 'option');
            else
    ...
    function get_xml_shop()
    {
        $s = '<shop>' . "\r\n";
            ...
            $s .= '<delivery-options>' . "\r\n";
            $s .= '<option cost="'.(int)Yii::app()->params['DELIVERY_COST'].'" days="3-4"/>' . "\r\n";
            $s .= '</delivery-options>' . "\r\n";

            //if(isset(Yii::app()->params['shopLocalDeliveryCost']) and !empty(Yii::app()->params['shopLocalDeliveryCost']))
            //    $s .= '<local_delivery_cost>'.Yii::app()->params['shopLocalDeliveryCost'].'</local_delivery_cost>'. "\r\n";
